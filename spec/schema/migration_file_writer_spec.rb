...
describe Chicago::Schema::MigrationFileWriter do
  before :each do
    @mock_db = mock(:db)
    @mock_db.should_receive(:database_type).and_return(:generic)
    @builder = Schema::MigrationFileWriter.new(@mock_db, "schema")
  end
  ...
  it "should write out a migration file generated by Sequel::MigrationBuilder" do
    ...
    Schema::Dimension.should_receive(:definitions).and_return([dimension])
    dimension.should_receive(:db_schema).and_return(:table => :definitions)
    Schema::Fact.should_receive(:definitions).and_return([])
