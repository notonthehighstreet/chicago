...
describe Chicago::FilterStringParser do
  ...
  it "should parse 'dimension.field:value'" do
    ...
    @dataset.opts[:where].should == Sequel::SQL::BooleanExpression.from_value_pairs(:year.qualify(:date) => "2010")
  end

  it "should parse 'dimension:value'" do
    @dataset = FilterStringParser.new("date:1").apply_to(@dataset)
    @dataset.opts[:where].should == Sequel::SQL::BooleanExpression.from_value_pairs(:original_id.qualify(:date) => "1")
  end

  it "should parse 'dimension.field:value1,value2'" do
    @dataset = FilterStringParser.new("date.year:2010,2009").apply_to(@dataset)
    @dataset.opts[:where].should == Sequel::SQL::BooleanExpression.from_value_pairs(:year.qualify(:date) => ["2010", "2009"])
  end
  ...
  it "should parse 'dimension.field:value;dimension.field:value'" do
    ...
    dataset.should_receive(:filter).with(:year.qualify(:date) => "2010").and_return(dataset)
    dataset.should_receive(:filter).with(:month.qualify(:date) => "May").and_return(dataset)
