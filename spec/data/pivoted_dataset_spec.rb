...
describe Chicago::Data::PivotedDataset do
  ...
  it "should accept a block to perform transformations on rows" do
    rows = Chicago::Data::PivotedDataset.new(@dataset, :month) do |row|
      row[:month] = row[:month].to_s
    end
    rows.to_a.should == [{:year => 2009, "1" => 1, "3" => 2, "4" => 3},
                         {:year => 2010, "1" => 4, "2" => 5, "4" => 6}]

    rows.pivot_columns.should == ["1", "2", "3", "4"]
  end
end
